...
def create_session_from_bound_draft(chat_id: int, actor_id: int) -> Optional[str]:
    ...
    if not draft:
        return None
    questions = get_draft_questions(draft["id"])
    if not questions:
        return None
    ...
    with closing(DBH.connect()) as conn:
        ...
        conn.commit()
    audit(actor_id, "create_session", session_id, {"chat_id": chat_id, "draft_id": draft["id"]})
    return session_id
...
async def finish_exam(context: ContextTypes.DEFAULT_TYPE, session_id: str, reason: str = "completed") -> None:
    ...
    if bound:
        ...
        delete_draft(bound["id"], int(session["created_by"]))
    else:
        ...
        with LOG_FILE.open("r", encoding="utf-8", errors="ignore") as fh:
            for line in fh.readlines()[-500:]:
                ...
                try:
                    ...
                    dt_obj = datetime.strptime(stamp, "%Y-%m-%d %H:%M:%S,%f").replace(tzinfo=timezone.utc).astimezone(TZ)
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if chat.type == "private" and command in {"panel", "help"}:
        if is_bot_admin(user.id):
            await send_panel(update, context)
        else:
...
async def run_scheduled_exam_job(context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if get_active_session(int(sched["chat_id"])):
        ...
        return
    session_id = create_session_from_bound_draft(int(sched["chat_id"]), int(sched["created_by"]))
    if not session_id:
...
def build_app() -> Application:
    ...
    app.add_handler(MessageHandler(filters.TEXT & ~filters.UpdateType.EDITED_MESSAGE, handle_text), group=2)
...
def main() -> None:
    threading.Thread(target=health_server, args=(CONFIG.port,), daemon=True).start()
    app = build_app()
