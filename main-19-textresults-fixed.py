...
def get_session_ranking(session_id: str) -> List[Dict[str, Any]]:
    ...
    ranking: List[Dict[str, Any]] = []
    total_questions = int(session["total_questions"])
    session_started = int(session["started_at"] or session["created_at"] or now_ts())
    for rank, row in enumerate(rows, start=1):
        answered = DBH.fetchone("SELECT COUNT(*) AS c, MIN(answered_at) AS first_ts, MAX(answered_at) AS last_ts FROM answers WHERE session_id=? AND user_id=?", (session_id, row["user_id"]))
        answered_count = int(answered["c"] if answered else 0)
        first_ts = int(answered["first_ts"] or 0) if answered else 0
        last_ts = int(answered["last_ts"] or 0) if answered else 0
        if first_ts and last_ts:
            duration = max(0, last_ts - first_ts)
        elif last_ts:
            duration = max(0, last_ts - session_started)
        elif int(row["last_answer_at"] or 0):
            duration = max(0, int(row["last_answer_at"]) - session_started)
        else:
            duration = 0
        full = " ".join(x for x in [row["first_name"], row["last_name"]] if x).strip()
        main_name, sub_name = split_user_labels(full or row["display_name"], row["username"], int(row["user_id"]))
...
def build_group_result_text(session: Dict[str, Any], ranking: List[Dict[str, Any]]) -> str:
    total = len(ranking)
    show_n = total if total < 20 else min(10, total)
    top = ranking[:show_n]
    q_count = int(session.get("total_questions") or 0)
    # old builds used the wrong key `per_question_seconds`; real session column is `question_time`
    per_q = int(session.get("question_time") or session.get("per_question_seconds") or 0)
    title = normalize_visual_text(session.get("title") or "Quiz")
    lines = [
        f"🏆 <b>Top results in the quiz</b> '<b>{html_escape(title)}</b>' 🔥",
        "",
    ...
    for item in top:
        label = item.get("name") or f"User {item.get('user_id','')}".strip()
        score = item.get("score", "0.00")
        time_label = item.get("time_label") or "0s"
        lines.append(f"{_medal_for_rank(int(item.get('rank', 0) or 0))} <b>{html_escape(str(label))}</b> — <b>{html_escape(str(score))}</b> ({html_escape(str(time_label))})")
        details = (
            f"🌟 Score: {html_escape(str(score))}\n"
    ...
    if total > show_n:
        lines.append(f"\n…and আরও <b>{total - show_n}</b> জন। Full analysis PDF inbox-এ পাঠানো হয়েছে।")
    return "\n".join(lines)
...
async def send_private_results(context: ContextTypes.DEFAULT_TYPE, session_id: str) -> None:
    ...
    for p in participants:
        ...
        text = (
            f"<b>{html_escape(normalize_visual_text(session['title']))}</b>\n"
            f"Your rank: <b>#{rank_item['rank']}</b>\n"
            f"⏱️ Time: <b>{html_escape(rank_item.get('time_label', '0s'))}</b>\n"
            f"✅ Correct: <b>{rank_item['correct']}</b>\n"
            f"❌ Wrong: <b>{rank_item['wrong']}</b>\n"
            f"➖ Skipped: <b>{rank_item['skipped']}</b>\n"
            f"🏁 Final Score: <b>{rank_item['score']}</b>\n\n"
...
async def send_admin_pdf_report(context: ContextTypes.DEFAULT_TYPE, session_id: str, ranking: List[Dict[str, Any]]) -> None:
    ...
    # compact ranking for PDF with readable names + time
    compact = []
    for r in ranking:
        name = r["name"]
        if r.get("sub_name"):
    ...
    pdf_bytes = render_report_pdf(session["title"], summary, compact, theme=theme)
    thumb_bytes = get_report_thumbnail_bytes(creator_id, session["title"])
    recipients: List[int] = []
    for uid in [creator_id] + list(CONFIG.owner_ids) + all_admin_ids():
        if uid not in recipients:
