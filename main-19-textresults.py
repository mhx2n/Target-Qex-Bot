...
def build_group_result_text(session: Dict[str, Any], ranking: List[Dict[str, Any]]) -> str:
    ...
    top = ranking[:show_n]
    q_count = int(session["total_questions"])
    per_q = int(session["per_question_seconds"])
    lines = [
        f"🏆 <b>Top results in the quiz</b> '<b>{html_escape(normalize_visual_text(session['title']))}</b>' 🔥",
        "",
    ...
    for item in top:
        ...
        lines.append(f"{_medal_for_rank(int(item['rank']))} <b>{html_escape(label)}</b> — <b>{html_escape(str(score))}</b> ({html_escape(time_label)})")
        details = (
            f"🌟 Score: {html_escape(str(score))}\n"
            f"✅ Correct: {item['correct']}\n"
            f"❌ Wrong: {item['wrong']}\n"
            f"⏭️ Skipped: {item['skipped']}"
        )
