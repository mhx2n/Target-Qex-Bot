...
def _render_rank_block(item: Dict[str, Any]) -> str:
    ...
    label = str(item.get("name") or f"User {item.get('user_id', '')}".strip())
...
def build_group_result_text(session: Dict[str, Any], ranking: List[Dict[str, Any]], *, full: bool = False) -> str:
    total = len(ranking)
    show_n = total if full or total < 20 else min(10, total)
    top = ranking[:show_n]
    ...
    for item in top:
        ...
        lines.append("")
    if total > show_n:
        lines.append(f"…আরও <b>{total - show_n}</b> জন। Full analysis inbox/PDF-এ পাঠানো হয়েছে।")
    return "\n".join(lines).strip()
