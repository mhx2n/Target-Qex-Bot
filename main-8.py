...
def build_commands_text(chat_type: str, is_admin_user: bool, is_owner_user: bool) -> str:
    ...
    if chat_type == "private":
        ...
        if is_owner_user:
            lines.extend(["", "<b>Owner</b>", "• /logs — owner gets log summary in PM", "• /restart — bot process restart"])
    return "\n".join(lines)
...
def render_report_pdf(title: str, summary: Dict[str, Any], ranking: List[Dict[str, Any]], qstats: List[Dict[str, Any]]) -> bytes:
    ...
    for page_index, chunk in enumerate(rank_chunks, start=1):
        ...
        for item in chunk:
            ...
            dr.text((200, y + 12), (item['name'] + (' ' + item.get('sub_name', '') if item.get('sub_name') else ''))[:42], font=body_font, fill="#F0F6FF")
...
async def ensure_fonts_hint_file() -> str:
    return (
        "fonts/ এর মধ্যে অন্তত এই 4টা file রাখুন: NotoSansBengali-Regular.ttf, "
...
async def send_temporary_group_warning(context: ContextTypes.DEFAULT_TYPE, chat_id: int, text: str) -> None:
    ...
    context.job_queue.run_once(
        ...
        when=max(30, CONFIG.delete_delay_seconds),
...
async def start_exam_countdown(context: ContextTypes.DEFAULT_TYPE, session_id: str, existing_message_id: Optional[int] = None) -> None:
    ...
    if existing_message_id:
        with suppress(TelegramError):
            await context.bot.edit_message_text(chat_id=chat_id, message_id=existing_message_id, text=build_text(countdown), parse_mode=ParseMode.HTML)
        class _M: pass
        msg = _M()
...
async def finish_exam(context: ContextTypes.DEFAULT_TYPE, session_id: str, reason: str = "completed") -> None:
    ...
    # always send pdf report to owner/admin inbox if possible
...
async def send_admin_pdf_report(context: ContextTypes.DEFAULT_TYPE, session_id: str, ranking: List[Dict[str, Any]]) -> None:
    ...
    recipients = []
    for uid in [int(session["created_by"])] + all_admin_ids():
...
async def send_panel(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    text = (
        ...
        f"4) Group-এ <code>.starttqex</code> দিলে active/bound draft auto use হবে\n"
        f"5) চাইলে <code>.schedule YYYY-MM-DD HH:MM</code>\n\n"
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    restricted_group_commands = {"binddraft", "examstatus", "stoptqex", "schedule", "listschedules", "schedules", "cancelschedule", "unschedule"}
    if command in restricted_group_commands and not await is_group_admin_or_global(update, context):
    ...
    if command == "starttqex":
        if get_active_session(chat.id):
            ...
            return
        draft = get_draft(args.strip()) if args.strip() else resolve_group_draft_for_actor(chat.id, user.id)
        if args.strip() and draft and int(draft['created_by']) not in OWNER_SET and int(draft['created_by']) != user.id and user.id not in all_admin_ids():
